...
/**
 * Sky_Trello_Api
 * This is a basic Sky_Trello_Api PHP wrapper that is used very similar to the Sky_Trello_Api-made client.js
 * library.  The method calls are the same (ie: Sky_Trello_Api->post() or Sky_Trello_Api->boards->get()).  See
 ...
 *
 * Sky_Trello_Api::authorize here does OAuth authentication, so you must pass your Secret Key to the
...
class Sky_Trello_Api
{
    ...
    /**
     * Sky_Trello_Api API Version
     */
    protected $apiVersion = 1;

    /**
     * Sky_Trello_Api API Endpoint
     */
    protected $apiEndpoint = 'https://api.trello.com';

    /**
     * Sky_Trello_Api Auth endpoint
     */
    ...
    /**
     * Consumer key from Sky_Trello_Api API
     */
     ...
     * version
     *
     * @return int Sky_Trello_Api API version
     */
    ...
    /**
     * authorize
     * Performs an OAuth authorization to Sky_Trello_Api.  Possible options include:
    ...
    public function authorize($userOptions = array(), $return = false)
    {
        ...
        $oauth = new Sky_Trello_OAuthSimple($this->consumer_key, $this->shared_secret);
        ...
        // Get a request token from Sky_Trello_Api
        $request = $oauth->sign(array(
     ...
     * @return \Sky_Trello_Api\Collection
     * @throws \Exception
     */
    ...
    /**
     * rest
     * This method actually performs the calls back to the Sky_Trello_Api REST service
     *
     * @param  string $method
     * @return mixed array of stdClass objects or false on failure
     * @throws \Exception
    ...
    public function rest($method)
    {
        ...
        //Mage::log($url, null, 'trello-url.log');
        //Mage::log($path, null, 'trello-url.log');
        //Mage::log(print_r($restData, true), null, 'trello-url.log');

        curl_setopt($ch, CURLOPT_URL, $url);

        // Grab the response from Sky_Trello_Api
        ...
        if (substr($responseCode, 0, 1) != '2') {
            ...
            // If we didn't get a 2xx HTTP response from Sky_Trello_Api, log the responsebody as an error
    ...
    public function buildRequestUrl($method, $path, $data)
    {
        ...
        if ($this->canOauth()) {
            ...
            //Mage::log($this->oauth_secret, null, 'hau.log');
            return $request['signed_url'];
        } else {
...
/**
 * Collection
 * This is a helper class for calling 'get' on collections (Sky_Trello_Api->boards->get())
...
class Collection
{
    ...
    /**
     * Sky_Trello_Api object so we can call REST api
     * @var \Sky_Trello_Api\Sky_Trello_Api
     */
    protected $trello;
     ...
     *
     * @param string $collection
     * @param \Sky_Trello_Api\Sky_Trello_Api $trello
     */
